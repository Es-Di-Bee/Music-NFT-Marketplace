import {
  Link,
  BrowserRouter,            // importing the 4 main components of react-router-dom. 
  Routes,                   // these components are responsible for navigation and routing between the components
  Route
...
function App() {
  // useState is used to keep track of a state in react
  // useState returns 2 arguments
  // one is the state itself and the other is the function which actually updates that state
  const [loading, setLoading] = useState(true)  // initializing the "loading" state with "true" so that the loading screen displays before making any wallet connection
  ...
  // these are the HTML codes which this "App" Component will return
  return (
    <BrowserRouter>
      <div className="App">
